...
├── src/
│   ├── data/
│   │   └── make_dataset.py                ← file code pipeline tiền xử lý
│   └── ...                                ← code phân tích (baseline, proposed, visualize)
│
└── requirements.txt
```
...
```bash
python src/data/make_dataset.py --raw_path /đường/dẫn/tới/file.csv
```
